...
const SocialButton = ({ icon: Icon, label }: any) => (
    <a href="#" aria-label={label} className="w-10 h-10 flex items-center justify-center bg-zinc-700/50 hover:bg-amber-600 rounded-lg transition duration-300">
        <Icon className="w-5 h-5 text-zinc-300 hover:text-white"/>
    </a>
);

const FooterLink = ({ href = "#", children }: any) => (
    <a href={href} className="text-zinc-400 hover:text-amber-500 transition duration-200 block mb-2">
        {children}
    </a>
);
    ...
    <div className="flex items-start mb-3">
        ...
        <div className="text-sm">
            {content.map((line: string, index: number) => (
                <p key={index} className="text-zinc-400 leading-relaxed">{line}</p>
            ))}
        </div>
    </div>
);
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          ...
          <div className="md:col-span-1">
            ...
            <nav>
              ...
              <FooterLink>Salon Services</FooterLink>
              <FooterLink>Academy Courses</FooterLink>
      ...
      <div className="border-t border-zinc-800 bg-[#111827] py-4">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row justify-between items-center text-sm text-zinc-500">
          <p className="mb-2 md:mb-0">
            © 2024 360 Salon & Academy. All rights reserved.
          </p>
